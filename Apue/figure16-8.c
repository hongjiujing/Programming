/*
 * 程序清单 16-8 基于数据报提供系统 uptime 的服务器程序
 *
 * 编译方法：
 *     $ cc err.c figure13-1.c figure16-3.c figure16-8.c -o ruptimed
 *
 * 使用方法：
 *     $ ./ruptimed
 *
...
serve(int sockfd)
{
  ...
  for (;;) {
    ...
    if ((n = recvfrom(sockfd, buf, BUFLEN, 0, (struct sockaddr *)abuf, &alen)) < 0) {
      syslog(LOG_ERR, "ruptimed: recvfrom: recvfrom error: %s", strerror(errno));
      exit(1);
    }
